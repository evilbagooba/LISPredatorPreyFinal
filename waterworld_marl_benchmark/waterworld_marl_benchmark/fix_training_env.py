...
class SingleAgentWrapper(gym.Env):
    ...
    def step(self, action):
        ...
        # 训练智能体使用传入的动作
        if self.train_agent_id in self.env.agents:
            actions[self.train_agent_id] = action
        
        # 对手使用固定策略
        for agent_id in self.env.agents:
            if agent_id != self.train_agent_id and agent_id in self.opponent_policies:
                policy = self.opponent_policies[agent_id]
                obs = self.env.observe(agent_id) if hasattr(self.env, 'observe') else None
