...
class AgentNameParser:
    ...
    def validate(name: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            valid_algorithms = ['ppo', 'sac', 'td3', 'a2c', 'random', 'heuristic']
...
# ============================================================================
# Policy Adapters & Factory for EXECUTION mode
# ============================================================================

from stable_baselines3.common.base_class import BaseAlgorithm

class BaseExecPolicy:
    """执行时用的统一策略接口"""
    def act(self, obs, action_space):
        raise NotImplementedError

class RandomPolicy(BaseExecPolicy):
    def act(self, obs, action_space):
        return action_space.sample()

class SB3Policy(BaseExecPolicy):
    """把 SB3 模型包装成执行策略"""
    ...
    def act(self, obs, action_space):
        # obs: (np.ndarray) 单一 agent 的观测
        action, _ = self.model.predict(obs, deterministic=self.deterministic)
        # 保险：若环境 action space 发生变化，裁剪一下
        if hasattr(action_space, "low") and hasattr(action_space, "high"):
            return np.clip(action, action_space.low, action_space.high)
        return action

class HeuristicEvasionPolicy(BaseExecPolicy):
    """可选：简单启发式（prey 反向远离最近 predator，大致方向推力；仅作占位演示）"""
    def act(self, obs, action_space):
        # 对 waterworld 的 obs 结构不了解时，用随机退化
        # 你可按自己环境的 obs 索引规则替换为可解释的“远离/靠近”向量
        a = action_space.sample()
        return a

class PolicyFactory:
    """
    根据 DualListConfigManager 的配置，为每个 agent 构建执行策略。
    - 缓存：同名同版本的 exe 策略只加载一次（共享模型）。
    """
    def __init__(self, config_manager: 'DualListConfigManager', model_base_dir: str):
        self.cm = config_manager
        self.model_base_dir = model_base_dir
        self._cache = {}  # name -> BaseExecPolicy

    def _load_sb3_model(self, algo_name: str, model_path: str) -> BaseAlgorithm:
        # 不需要知道训练超参，SB3 的 .zip 自带
        ModelClass = get_algorithm_config(algo_name).get_model_class()
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
        return ModelClass.load(model_path, device=device)

    def _build_policy_from_cfg(self, cfg: Dict[str, Any]) -> BaseExecPolicy:
        name = cfg['name']                  # e.g. ppo_prey_v1_exe / random_prey_exe
        algo = cfg['algorithm']             # ppo / sac / td3 / a2c / random / heuristic
        mode = cfg['mode']                  # train / exe

        # 训练模式在“执行-only”里一般不会出现；兜底给随机
        if mode != 'exe':
            return RandomPolicy()
        ...
        if algo == 'heuristic':
            return HeuristicEvasionPolicy()

        # 其他（ppo/sac/td3/a2c）需要从 zip 加载
        model_path = AgentNameParser.get_model_path(name, self.model_base_dir)
        if (model_path is None) or (not os.path.exists(model_path)):
            print(f"⚠️  Model not found for '{name}', fallback to Random: {model_path}")
            return RandomPolicy()

        try:
            model = self._load_sb3_model(algo, model_path)
            return SB3Policy(model, deterministic=True)
        except Exception as e:
            print(f"⚠️  Failed to load model '{model_path}': {e} -> fallback Random")
            return RandomPolicy()

    def build_policies(self) -> List[BaseExecPolicy]:
        """
        为每个全局 agent（prey 全在前、predator 全在后）生成一个策略实例（或共享）。
        返回长度 == n_total 的策略列表。
        """
        policies: List[BaseExecPolicy] = []

        # prey（环境顺序在前）
        for cfg in self.cm.prey_configs:
            name = cfg['name']
            if name not in self._cache:
                self._cache[name] = self._build_policy_from_cfg(cfg)
            policies.append(self._cache[name])

        # predator（环境顺序在后）
        for cfg in self.cm.predator_configs:
            name = cfg['name']
            if name not in self._cache:
                self._cache[name] = self._build_policy_from_cfg(cfg)
            policies.append(self._cache[name])
...
def main(
    ...
    if not has_training or execution_mode:
        ...
        print(f"Running {n_episodes} episodes...")
            # 按配置构建每个 agent 的执行策略（顺序：prey 在前，predator 在后）
        policy_factory = PolicyFactory(config_manager, model_base_dir)
        exec_policies = policy_factory.build_policies()  # len == n_total    
        # TensorBoard设置
...
if __name__ == "__main__":
    ...
    prey_configs = [
        (3, 'ppo_prey_v1_exe'),   # 30个训练PPO
        (3, 'random_prey_exe'),     # 20个随机
    ]
