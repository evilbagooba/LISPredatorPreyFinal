...
class DualListConfigManager:
    ...
    def get_agent_config(self, agent_id: int) -> Dict[str, Any]:
        # 先是 prey，再是 predator
        if agent_id < self.n_preys:
            return self.prey_configs[agent_id].copy()
        else:
            return self.predator_configs[agent_id - self.n_preys].copy()
...
if __name__ == "__main__":
    ...
    # ============================================================
    predator_configs = [
        (3, 'ppo_predator_v1_train'),  # 3个训练PPO捕食者
    ]
    
    prey_configs = [
        (9, 'random_prey_exe'),   # 9个随机猎物
        (9, 'random_prey_exe'),   # 9个随机猎物
    ]
