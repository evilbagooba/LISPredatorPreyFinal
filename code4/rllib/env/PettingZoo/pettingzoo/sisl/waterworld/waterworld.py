...
class raw_env(AECEnv, EzPickle):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 关键修改：生成新的智能体名称
        predator_agents = [f"predator_{i}" for i in range(self.env.n_predators)]
        prey_agents = [f"prey_{i}" for i in range(self.env.n_preys)]
        self.agents = predator_agents + prey_agents
    ...
    def reset(self, seed=None, options=None):
        ...
        # print(f"  _cumulative_rewards keys: {list(self._cumulative_rewards.keys())}")
    def close(self):
        if self.has_reset:
            self.env.close()
    ...
    def step(self, action):
        ...
        if self.terminations[self.agent_selection] or self.truncations[self.agent_selection]:
            print("🔥 Warning: Agent is already terminated or truncated, skipping step.")
        ...
        if is_last:
            ...
            # ✅ 使用 base 环境缓存的 step_infos（indicators 还未清零时生成的）
            for agent in self.agents:
                if agent in self.agent_name_mapping:
                    agent_id = self.agent_name_mapping[agent]
                    # 优先使用缓存，如果没有缓存才实时生成
                    if hasattr(self.env, '_cached_step_infos') and agent_id in self.env._cached_step_infos:
                        self.infos[agent] = self.env._cached_step_infos[agent_id]
                    else:
                        self.infos[agent] = self.env.get_step_info(agent_id)

            #     # 确保死亡的代理奖励为 0
            # for agent in self.rewards:
            #     if self.terminations[agent]:  # _is_dead 可以检查健康或其他条件
            #         self.rewards[agent] = 0  # 死亡的代理奖励设为 0
...
# <--to do change the step function#>
    # def step(self, action):
    #     # 🔥 添加安全检查
    #     # if self.agent_selection not in self._cumulative_rewards:
    #     #     print(f"ERROR: Missing agent {self.agent_selection} in _cumulative_rewards")
    #     #     self._cumulative_rewards[self.agent_selection] = 0
    #     if (
    #         self.terminations[self.agent_selection]
    #         or self.truncations[self.agent_selection]
    ...
    #     agent = self.agent_selection
    #     is_last = self._agent_selector.is_last()
    #     # # 🔥 安全检查agent_name_mapping
    #     # if agent not in self.agent_name_mapping:
    #     #     raise ValueError(f"Agent {agent} not found in agent_name_mapping")
    ...
    #             self.rewards[r] += self.env.last_rewards[self.agent_name_mapping[r]]
    #         for agent in self.terminations:
    #             self.terminations[agent] = bool(self.env.get_last_dones()[
    #                 self.agent_name_mapping[agent]
    #             ])
    #         self.agent_name_mapping = {
    #             agent: i
    ...
    #     #     self.truncations = dict(zip(self.agents, [True for _ in self.agents]))
    #     # else:
    #     #     self.terminations = dict(zip(self.agents, [bool(x) for x in self.env.last_dones]))
    #     # assert isinstance(self.terminations[agent], bool), "terminated from last is not True or False"
    #     # assert isinstance(self.truncations[agent], bool), "terminated from last is not True or False"
    #     # self._cumulative_rewards[self.agent_selection] = 0
    #     # self.agent_selection = self._agent_selector.next()
    #     # self._accumulate_rewards()
