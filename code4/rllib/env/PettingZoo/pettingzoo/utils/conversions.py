...
class aec_to_parallel_wrapper(ParallelEnv[AgentID, ObsType, ActionType]):
    ...
    def step(self, actions):
        ...
        assert all(isinstance(v, bool) for v in terminations.values()), "Some terminations are not bool"
...
class parallel_to_aec_wrapper(AECEnv[AgentID, ObsType, Optional[ActionType]]):
    ...
    def step(self, action: Optional[ActionType]):
        ...
        if self._agent_selector.is_last():
            ...
            terminations = {k: bool(v) for k, v in terminations.items()}
            truncations = {k: bool(v) for k, v in truncations.items()}
