...
def get_agents(
    ...
    for agent_name in agents:
        algo = agent_algorithms.get(agent_name, "fault")

        if algo == "PPO":
            # 创建连续动作空间的 Actor 网络
            class ContinuousPPOActorNet(torch.nn.Module):
                ...
                def forward(self, obs, state=None, info={}):
                    """连续动作Actor网络前向传播 - 返回 ([mu, log_std], state)"""
            ...
            # Critic网络保持不变，因为它只输出值函数
            class ContinuousPPOCriticNet(torch.nn.Module):
            ...
            # 创建连续动作PPO网络
            actor_net = ContinuousPPOActorNet(
                args.state_shape,
            ...
            critic_net = ContinuousPPOCriticNet(
                args.state_shape,
                hidden_sizes=args.hidden_sizes,
                device=args.device,
            ).to(args.device)
            ...
            if agent_name in agent_resume_paths:
                ...
                try:
                    print(f"Loading continuous PPO pretrained model for {agent_name} from {resume_path}")
                    policy.load_state_dict(torch.load(resume_path, map_location=args.device))
                    print(f"Successfully loaded continuous PPO model for {agent_name}")
                except Exception as e:
                    print(f"Warning: Failed to load continuous PPO model for {agent_name}: {e}")

            print(f"Continuous PPO policy created for {agent_name}")
            ...
            # 创建连续动作空间的 TRPO Actor 网络
            class ContinuousTRPOActorNet(torch.nn.Module):
                def __init__(self, state_shape, action_shape, hidden_sizes, device="cpu", 
                            log_std_init=-0.5, log_std_min=-10.0, log_std_max=2.0):
                    super().__init__()
                    self.output_shape = action_shape
                ...
                def forward(self, obs, state=None, info={}):
                    """连续动作TRPO Actor网络前向传播 - 返回 ([mu, log_std], state)"""
                    obs_data = self._extract_obs(obs)
                    features = self.feature_net(obs_data)
            ...
            # 创建连续动作空间的 TRPO Critic 网络
            class ContinuousTRPOCriticNet(torch.nn.Module):
                def __init__(self, state_shape, hidden_sizes, device="cpu"):
                    super().__init__()
                ...
                def forward(self, obs, **kwargs):
                    """连续动作TRPO Critic网络前向传播 - 只返回tensor"""
                    obs_data = self._extract_obs(obs)
                    value = self.net(obs_data)
                    return value
            ...
            # 创建连续动作TRPO网络
            actor_net = ContinuousTRPOActorNet(
                args.state_shape,
                args.action_shape,  # 对于Box(2,)，这应该是2
                hidden_sizes=args.hidden_sizes,
                device=args.device,
            ...
            critic_net = ContinuousTRPOCriticNet(
                args.state_shape,
                hidden_sizes=args.hidden_sizes,
                device=args.device,
            ).to(args.device)

            # 优化器：TRPO的Actor使用特殊的自然梯度更新，Critic使用普通梯度下降
            # Actor优化器设置学习率为0，因为TRPO会手动更新参数
            actor_optimizer = torch.optim.Adam(actor_net.parameters(), lr=0.0)
            ...
            policy = TRPOPolicy(
                ...
                # TRPO特有参数
                max_kl=args.max_kl,                  # KL散度约束，连续动作通常使用更小的值
                backtrack_coeff=0.8,                 # 回溯系数，TRPO线搜索用
                max_backtracks=args.max_backtracks,  # 最大回溯次数
                
                # 从NPGPolicy继承的参数
                advantage_normalization=args.advantage_normalization,  # 优势标准化
                optim_critic_iters=args.optim_critic_iters,           # Critic优化次数
            ...
            if agent_name in agent_resume_paths:
                resume_path = agent_resume_paths[agent_name]
                try:
                    print(f"Loading continuous TRPO pretrained model for {agent_name} from {resume_path}")
                    policy.load_state_dict(torch.load(resume_path, map_location=args.device))
                    print(f"Successfully loaded continuous TRPO model for {agent_name}")
                except Exception as e:
                    print(f"Warning: Failed to load continuous TRPO model for {agent_name}: {e}")

            print(f"Continuous TRPO policy created for {agent_name}")
            policies[agent_name] = policy
            pass  # 后续扩展
        elif algo == "NPG":
            # ==================== 连续动作空间NPG算法实现 ====================
...
def get_env(render_mode=None):
    ...
    agent_algos = ["Random", "Random", "Random", "Random"] * 50  # 给足够多的PPO算法
    env = waterworld_v4.env(
        render_mode=render_mode,
        n_predators=2,
        n_preys=2,   # 减少数量便于调试
        n_evaders=5, # 减少数量便于调试
        n_obstacles=2,
        obstacle_coord=[(0.2, 0.2), (0.8, 0.2)],
        n_poisons=10,  # 减少数量便于调试
...
class PerfTensorboardLogger(TensorboardLogger):
    def __init__(self, writer: SummaryWriter, test_collector, ema_beta: float = 0.0,
        ...
        # 添加累积奖励追踪器
        self.acc_reward_tracker = AccumulatedRewardTracker()
    ...
    def log_train_data(self, data: dict, step: int) -> None:
        ...
        # 记录累积奖励统计
        self._log_accumulated_rewards(step, is_training=True)
    ...
    def log_test_data(self, data: dict, step: int) -> None:
        ...
        # 记录累积奖励统计
        self._log_accumulated_rewards(step, is_training=False)
        
        self.writer.flush()

    def _log_accumulated_rewards(self, step: int, is_training: bool = True):
        """记录累积奖励到TensorBoard - 修复版本"""
        prefix = "Train" if is_training else "Test"
        
        # 获取全局累积奖励和episode统计
        global_acc_rewards = self.acc_reward_tracker.get_global_accumulated_rewards()
        current_episode_rewards = self.acc_reward_tracker.get_current_episode_rewards()
        episode_stats = self.acc_reward_tracker.get_episode_statistics()
        
        # 记录全局累积奖励（这个应该是单调递增的）
        for agent, acc_reward in global_acc_rewards.items():
            role = "Predator" if agent.startswith("predator") else "Prey"
            algo = self.agent_algo_map.get(agent, "UNK")
            self.writer.add_scalar(f"AccReward/{prefix}/{role}/{algo}/Global_Accumulated", acc_reward, step)
        
        # 记录当前episode奖励
        for agent, episode_reward in current_episode_rewards.items():
            role = "Predator" if agent.startswith("predator") else "Prey"
            algo = self.agent_algo_map.get(agent, "UNK")
            self.writer.add_scalar(f"AccReward/{prefix}/{role}/{algo}/Current_Episode", episode_reward, step)
        ...
        for agent, stats in episode_stats.items():
            ...
            self.writer.add_scalar(f"AccReward/{prefix}/{role}/{algo}/Episode_Mean", stats['mean'], step)
            self.writer.add_scalar(f"AccReward/{prefix}/{role}/{algo}/Episode_Count", stats['count'], step)
            if stats['count'] > 1:  # 只在有多个episode时记录std
                self.writer.add_scalar(f"AccReward/{prefix}/{role}/{algo}/Episode_Std", stats['std'], step)
        
        # 整体统计
        if global_acc_rewards:
            predator_global = [v for k, v in global_acc_rewards.items() if k.startswith("predator")]
            prey_global = [v for k, v in global_acc_rewards.items() if k.startswith("prey")]
            
            if predator_global:
                self.writer.add_scalar(f"AccReward/{prefix}/Overall/Predator_Global_Mean", np.mean(predator_global), step)
            if prey_global:
                self.writer.add_scalar(f"AccReward/{prefix}/Overall/Prey_Global_Mean", np.mean(prey_global), step)
    def update_accumulated_rewards(self, agent_rewards, dones=None):
        """外部调用接口，用于更新累积奖励"""
        self.acc_reward_tracker.update(agent_rewards, dones)
...
def train_agent(
    ...
    """
    PPO专用训练函数 - 合并 writer，保证 AccReward 写到同一个 run
    """
    # ======== environment setup =========
    ...
    class AccRewardCollector(Collector):
        ...
        def collect(self, n_step=None, n_episode=None, **kwargs):
            result = super().collect(n_step=n_step, n_episode=n_episode, **kwargs)
            
            # 简化日志，只在有实际数据时处理
            if hasattr(result, 'rews') and result.rews is not None:
                rews_matrix = result.rews
                
                # 确保智能体名称已设置且数量匹配
                if not self.agent_names:
                    return result
                
                if len(self.agent_names) != rews_matrix.shape[1]:
                    print(f"Warning: Agent names count ({len(self.agent_names)}) doesn't match rewards columns ({rews_matrix.shape[1]})")
                    return result
                
                # 处理每个episode的奖励
                for episode_idx in range(rews_matrix.shape[0]):
                    episode_rewards = {}
                    
                    for agent_idx, agent_name in enumerate(self.agent_names):
                        episode_rewards[agent_name] = float(rews_matrix[episode_idx, agent_idx])
                    
                    # 只更新步骤奖励，不处理episode结束
                    self.acc_reward_tracker.update_step_reward(episode_rewards)
                    
                    # 如果有logger，同步更新
                    if self.logger and hasattr(self.logger, 'acc_reward_tracker'):
                        self.logger.acc_reward_tracker.update_step_reward(episode_rewards)
                
                # 在每次collect结束后，标记episode完成
                if n_episode and n_episode > 0:
                    self.acc_reward_tracker.complete_episode(self.agent_names)
                    if self.logger and hasattr(self.logger, 'acc_reward_tracker'):
                        self.logger.acc_reward_tracker.complete_episode(self.agent_names)
            
            return result


    # ======== collectors =========
    train_collector = AccRewardCollector(policy, train_envs,
                                        buffer=VectorReplayBuffer(args.buffer_size, args.training_num),
                                        exploration_noise=False)
    test_collector = AccRewardCollector(policy, test_envs,
                                        buffer=VectorReplayBuffer(max(50000, args.buffer_size // 2), args.test_num),
                                        exploration_noise=False)

    # 关键修复：显式设置正确的智能体名称
    train_collector.set_agent_names(agents)  # agents = ['predator_0', 'predator_1', 'prey_0', 'prey_1']
    test_collector.set_agent_names(agents)
    ...
    # 重要：logger 要在任何 collect 之前挂上
    train_collector.set_logger(logger)
    test_collector.set_logger(logger)

    # ======== 初始采样（现在放在 logger 挂好之后）=========
    initial_collect_steps = max(args.step_per_collect, 200)
    train_collector.collect(n_step=initial_collect_steps)

    def reward_metric(rews):
        # 基本索引和计步
        pred_idx = [i for i, n in enumerate(reward_metric.agent_order) if n.startswith("predator")]
        prey_idx = [i for i, n in enumerate(reward_metric.agent_order) if n.startswith("prey")]
        result = rews[:, pred_idx].mean(axis=1)

        if not hasattr(reward_metric, "step"):
            reward_metric.step = 0
        reward_metric.step += 1
        step = reward_metric.step
        
        # 只记录即时奖励，不处理累积奖励（交给collector处理）
        if pred_idx:
            reward_metric.writer.add_scalar('Reward/Test/Predator_Immediate', rews[:, pred_idx].mean(), step)
        if prey_idx:
            reward_metric.writer.add_scalar('Reward/Test/Prey_Immediate', rews[:, prey_idx].mean(), step)

        # 分组即时奖励
        group_vals = defaultdict(list)
        for col, agent_name in enumerate(reward_metric.agent_order):
            if col < rews.shape[1]:
                role = "Predator" if agent_name.startswith("predator") else "Prey"
                algo = reward_metric.agent_algo_map.get(agent_name, "UNK")
                group_vals[(role, algo)].append(rews[:, col])
        
        for (role, algo), arrs in group_vals.items():
            val = np.mean([a.mean() for a in arrs])
            reward_metric.writer.add_scalar(f"Reward/Test/{role}/{algo}/Immediate", val, step)
        
        reward_metric.writer.flush()
        return result
    # 将必要的上下文注入 reward_metric
    reward_metric.agent_order = agent_order
    reward_metric.agent_algo_map = agent_algo_map
    reward_metric.test_collector = test_collector
    reward_metric.writer = writer        # <<< 关键：复用同一个 writer
